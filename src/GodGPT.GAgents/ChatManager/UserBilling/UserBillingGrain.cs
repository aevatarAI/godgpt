...
public class UserBillingGrain : Grain<UserBillingState>, IUserBillingGrain
{
    ...
    public async Task<bool> HandleAppStoreNotificationAsync(Guid userId, string jsonPayload)
    {
        try
        {
            ...
            // 6. If userId is empty and we have AppAccountToken in the transaction info, use it
            ...
            switch (notificationTypeEnum)
            {
                ...
                case AppStoreNotificationType.DID_CHANGE_RENEWAL_STATUS:
                    ...
                    {
                        _logger.LogInformation("[UserBillingGrain][HandleAppStoreNotificationAsync] Auto-renewal disabled");
                    }
                ...
                case AppStoreNotificationType.EXPIRED:
                    ...
                    switch (subtypeEnum)
                    {
                        case AppStoreNotificationSubtype.VOLUNTARY:
                            _logger.LogInformation("[UserBillingGrain][HandleAppStoreNotificationAsync] Subscription expired voluntarily");
                            break;
                        case AppStoreNotificationSubtype.BILLING_RETRY:
                            _logger.LogInformation("[UserBillingGrain][HandleAppStoreNotificationAsync] Subscription expired after billing retry period");
                            break;
                        case AppStoreNotificationSubtype.PRICE_INCREASE:
                            _logger.LogInformation("[UserBillingGrain][HandleAppStoreNotificationAsync] Subscription expired due to price increase");
                            break;
                        case AppStoreNotificationSubtype.PRODUCT_NOT_FOR_SALE:
                            _logger.LogInformation("[UserBillingGrain][HandleAppStoreNotificationAsync] Subscription expired as product not for sale");
                            break;
                    }
                ...
                case AppStoreNotificationType.REVOKE:
                    _logger.LogInformation("[UserBillingGrain][HandleAppStoreNotificationAsync] Family Sharing purchase revoked");
    ...
    public async Task<VerifyReceiptResponseDto> VerifyAppStoreTransactionAsync(VerifyReceiptRequestDto requestDto, bool savePaymentEnabled)
    {
        try
        {
            ...
            if (!string.IsNullOrEmpty(requestDto.UserId) && Guid.TryParse(requestDto.UserId, out var userId) && savePaymentEnabled)
            {
                ...
                await CreateAppStoreSubscriptionAsync(userId, purchaseDate, transactionInfo);
            }
    ...
    private async Task CreateAppStoreSubscriptionAsync(Guid userId, DateTime purchaseDate, AppStoreJWSTransactionDecodedPayload appleResponse)
    {
        ...
        {
            _logger.LogDebug("[UserBillingGrain][VerifyAppStoreTransactionAsync] cancel stripe subscription, userId: {0}, subscriptionId: {1}, cancel: {2}",
        ...
        if (appleProduct.IsUltimate)
        {
            ...
            {
                _logger.LogDebug("[UserBillingGrain][VerifyAppStoreTransactionAsync] cancel stripe premiumSubscription, userId: {0}, subscriptionId: {1}, cancel: {2}",
                    userId, appleResponse.OriginalTransactionId, JsonConvert.SerializeObject(premiumSubscriptionDto.SubscriptionIds));
