...
public class WesternAstrologyCalculator
{
    ...
    /// <param name="birthTime">Birth time</param>
    /// <param name="latitude">Latitude in decimal degrees</param>
    /// <param name="longitude">Longitude in decimal degrees</param>
    public Task<(string sunSign, string moonSign, string risingSign)> CalculateSignsAsync(
        DateOnly birthDate,
        TimeOnly birthTime,
        double latitude,
        double longitude)
    {
        try
        {
            _logger.LogInformation($"[WesternAstrology] Calculating signs for coordinates ({latitude}, {longitude})");
            ...
            return Task.FromResult((sunSign, moonSign, risingSign));
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, $"[WesternAstrology] Failed to calculate signs for coordinates ({latitude}, {longitude})");
            // Fallback: at least return Sun sign based on date
            string sunSign = CalculateSunSignSimple(birthDate);
            return Task.FromResult((sunSign, sunSign, sunSign)); // Use Sun sign as fallback for all
        }
    }
