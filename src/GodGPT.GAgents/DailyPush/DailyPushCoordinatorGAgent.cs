    ...
    IDailyPushCoordinatorGAgent, IRemindable
{
    ...
    public async Task<List<TimezoneDeviceInfo>> GetDevicesInTimezoneAsync()
    {
        ...
        try
        {
                ...
                if (userBatch.Any())
                {
                    ...
                    var userDeviceTasks = userBatch.Select(async userId =>
                    {
                        try
                        {
                            ...
                            var hasEnabledDeviceInTimezone =
                                await chatManagerGAgent.HasEnabledDeviceInTimezoneAsync(_timeZoneId);
            ...
            _logger.LogInformation(
                "Retrieved {DeviceCount} device entries for {UserCount} users in timezone {TimeZone}",
    ...
    public async Task<InstantPushResult> SendInstantPushAsync()
    {
        try
        {
            ...
            _logger.LogInformation("📱 Found {UserCount} active users in timezone {TimeZone}", activeUsers.Count,
                _timeZoneId);
            ...
            try
            {
                ...
                var contentService =
                    ServiceProvider.GetService(typeof(DailyPushContentService)) as DailyPushContentService;
                if (contentService != null)
                {
                    ...
                    if (csvContents?.Count >= 2)
                    {
                        ...
                        foreach (var csvContent in selectedContents)
                        {
                            ...
                            if (!string.IsNullOrEmpty(csvContent.TitleEn) ||
                                !string.IsNullOrEmpty(csvContent.ContentEn))
                            {
                            ...
                            Logger.LogDebug(
                                "🔍 CSV Traditional Chinese content check: Key={ContentKey}, TitleZh='{TitleZh}', ContentZh='{ContentZh}'",
                                csvContent.ContentKey, csvContent.TitleZh, csvContent.ContentZh);

                            if (!string.IsNullOrEmpty(csvContent.TitleZh) ||
                                !string.IsNullOrEmpty(csvContent.ContentZh))
                            {
                                ...
                                Logger.LogInformation(
                                    "✅ Added Traditional Chinese content for key={ContentKey}: Title='{Title}'",
                                    csvContent.ContentKey, csvContent.TitleZh);
                            }
                            else
                            {
                                Logger.LogWarning(
                                    "⚠️ No Traditional Chinese content available for key={ContentKey}, skipping zh-tw",
                                    csvContent.ContentKey);
                            }

                            // Add Simplified Chinese content if available
                            if (!string.IsNullOrEmpty(csvContent.TitleZhSc) ||
                                !string.IsNullOrEmpty(csvContent.ContentZhSc))
                            {
                            ...
                            Logger.LogDebug(
                                "🔍 CSV Spanish content check: Key={ContentKey}, TitleEs='{TitleEs}', ContentEs='{ContentEs}'",
                                csvContent.ContentKey, csvContent.TitleEs, csvContent.ContentEs);

                            if (!string.IsNullOrEmpty(csvContent.TitleEs) ||
                                !string.IsNullOrEmpty(csvContent.ContentEs))
                            {
                        ...
                        _logger.LogInformation("✅ Successfully loaded {Count} contents from CSV for instant push",
                            testContent.Count);
                    }
                    else
                    {
                        _logger.LogWarning(
                            "⚠️ Not enough content in CSV file ({Count} available, need 2), using fallback test content",
                            csvContents?.Count ?? 0);
            ...
            foreach (var userId in activeUsers)
            {
                try
                {
                    ...
                    if (!hasEnabledDevice)
                    {
                        _logger.LogDebug("User {UserId} has no enabled devices in timezone {TimeZone}", userId,
                            _timeZoneId);
                        continue;
                    }
                    ...
                    var enabledDevicesInTimezone =
                        userDevices.Where(d => d.PushEnabled && d.TimeZoneId == _timeZoneId).ToList();
            ...
            {
                _logger.LogInformation(
                    "🔍 InstantPush timezone deduplication: Removed {DuplicateCount} duplicate pushTokens in timezone {TimeZone}",
                    duplicateCount, _timeZoneId);
            }

            _logger.LogInformation(
                "🧪 InstantPush: Processing {DeviceCount} unique devices (deduplicated from {TotalDevices}) in timezone {TimeZone}",
            ...
            foreach (var (userId, device) in deduplicatedDevices)
            {
                try
                {
                    ...
                    _logger.LogDebug("✅ Sent instant push to user {UserId} for device {DeviceId}", userId,
                        device.DeviceId);
                }
                catch (Exception ex)
                {
            ...
            _logger.LogInformation(
                "🎉 Instant push completed for timezone {TimeZone}: {TotalUsers} users, {TotalDevices} devices, {SuccessfulPushes} successful, {FailedPushes} failed",
            ...
            new DailyNotificationContent
            {
                ...
                LocalizedContents = new Dictionary<string, LocalizedContentData>
                {
                    ...
                    ["es"] = new LocalizedContentData
                    {
                        ...
                        Content =
                            $"Este es un mensaje de prueba de push instantáneo, enviado en: {DateTime.Now:yyyy-MM-dd HH:mm:ss}"
                    },
            ...
            new DailyNotificationContent
            {
                ...
                LocalizedContents = new Dictionary<string, LocalizedContentData>
                {
                    ...
                    ["es"] = new LocalizedContentData
                    {
                        ...
                        Content =
                            $"Este es el segundo mensaje de prueba de push instantáneo, enviado en: {DateTime.Now:yyyy-MM-dd HH:mm:ss}"
                    },
                    ["en"] = new LocalizedContentData
                    {
                        Title = "🧪 Instant Push Test #2",
                        Content =
                            $"This is the second instant push test message, sent at: {DateTime.Now:yyyy-MM-dd HH:mm:ss}"
                    }
                },
                IsActive = true
            }
    ...
    {
        _logger.LogWarning(
            "🧹 Starting emergency cleanup for potentially orphaned grain. Current timezone: '{TimeZone}', GrainId: {GrainId}",
        ...
        try
        {
            ...
            _logger.LogWarning(
                "✅ Emergency cleanup completed for grain {GrainId}. Removed {ReminderCount} reminders. Grain is now inactive and ready for proper initialization.",
